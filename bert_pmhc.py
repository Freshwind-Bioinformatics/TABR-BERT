...
class BERT(nn.Module):
    def __init__(self, n_layers=4, d_model=256, n_heads=8, \
                 maxlen=54, n_segments=2, vocab_size=25, device="cuda"):
    ...
    def forward(self, input_ids, segment_ids, masked_pos="False"):
        ...
        h_pooled = output.sum(dim=1)
        logits_clsf = self.classifier(h_pooled) # [batch_size, 2] predict isNext

        # prediction mode
        if masked_pos == "False":
            return logits_clsf, output
